...
# 三、怎么编码
## 1. ASCII
一种直观的想法就是，向编字典一样，制作一个映射表，把人类语言和计算机二进制对应起来就行了。这样的思想就孕育出了ASCII编码（因为是美国人设计的，所以只有常用英文字符），如下表所示：
...
于是中国自己搞了一套区位码的概念，来支持中文语言体系，并由此诞生了GBK编码。但是这套编码和国际通行的各种编码又不兼容了，更加加剧了混乱的局面。

人们就在思考，有没有更好的办法？

## 2. Unicode
于是 Unicode 应运而生，它的使命很宏大：编码这个星球上有史以来所有的的字符。其实本质思想其实和 ASCII 并无二致，它只是一张更大的映射表。

### 2.1 Code Point 和 Plane 
Unicode 为每个字符分配了一个全局唯一识别码，称为：Code Point。
取值区间为：U+0000~U+10FFFF （Code Point 以 U+ 前缀），总共可编码 1114112 个字符。现行最新版本 Unicode 12.1，一共分配了 137,994 个字符。

Code Point 一共划分为17个区域，称为平面（Plane），每个平面分配了16位，可以容纳 2^16 = 65536 个字符。Plane0 被称为BMP（Basic Multilingual Plane），涵盖了现在世界上的绝大多数语言。  
小贴士：看 Code Point 的 U+??XXXX 的 ?? 部分，就能知道它落在哪个平面上。比如：U+597D 在0号平面，即BMP，U+2B7D2 就是2号平面。
...
可以看出现在用的比较多的集中在前三个平面上，它们的分配情况如下所示（蓝色部分的汉字占据了好大一块区域，常用字在BMP，生僻字在Plane2）：  
![Unicode Plane 0-2]({{ site.baseurl }}/assets/images/posts/plane0-2.png)

再来看一下前三个平面的使用热度图(Plane1下面那一小块活跃区是emoji)：  
![Unicode Plane 0-2 热度]({{ site.baseurl }}/assets/images/posts/plane0-2_hotness.png)

### 2.2 Code Point 存储方案
Code Point 只是一个概念定义，并没有规定实际如何存储，所以在具体存储方案上，出现了不同的方式：
...
变长编码的核心设计思想是：根据不同情况，灵活使用不同长度的字节存储 Code Point。  
如果是常用字符，使用1-2个字节来存储，非常用字符使用更多的字节来存，这样一来就解决了存储效率过低的问题，Unicode 才真正普及开来。
...
UTF-16编码实例：
* 编码`好`这个字符（常用字，在 BMP 上）：
1. 查 Unicode 表可知，Code Point 值为 U+597D
2. 该值落在 0x0000-0xFFFF 中，根据上表 UTF-16 编码规则，此时可以直接存储，因此编码之后的值为 0x597D。

* 编码`𫟒`这个字符（一个古汉语生僻字，在 Plane 2上），编码过程示意图如下：  
![UTF-16编码规则]({{ site.baseurl }}/assets/images/posts/utf-16-2.png)  
1. 查 Unicode 表可知，Code Point 值为 U+2B7D2
2. 该值落在 0x10000-0x10FFFF 中，根据上表 UTF-16 编码规则，此时需要拆分为两个编码单元进行存储
...
Code Point 规则:
* 对于单字节的符号，字节的第一位设为0，后面7位为这个符号的 Code Point 码。因此对于英语字母，UTF-8 编码和 ASCII 码是相同的，这个特性对于原先使用 ASCII 编码的文本来说非常友好。
* 对于n字节的符号（1 < n <= 4），第一个字节的前n位都设为1，第n + 1位设为0，后面字节的前两位一律设为10。剩下的没有提及的二进制位，共同组成这个符号的 Code Point。

UTF-8 编码实例，继续以`𫟒`为例，编码过程示意图如下：  
![UTF-8编码规则]({{ site.baseurl }}/assets/images/posts/utf-8-2.png)  
1. 查 UTF-8 规则表，Code Point 落在 0x10000-0x10FFFF，因此需要4个字节存储
2. 转为二进制：0010 1011 0111 1101 0010
3. 放到 UTF-8 的四个字节里：11110 000 10101011 10011111 10010010
4. 用16进制展示位：0xF0AB 0x9F92
...
于是字符 `好`，经过 UTF-16 编码，再使用 UTF-8 解码，得到了 `Y}` 这么一个奇怪的结果，可以拿`𫟒`进行相同的测试，
结果发现有些 UTF-16 编码出来的字节根本不符合 UTF-8 规则，无法解码，会被展示为未知字符：`�m��`，这就是我们通常所说的乱码了。
...
如下代码验证了上述逻辑，同时也说明 String 的 length 方法返回的是 char 的个数，并非 Unicode 字符个数；toCharArray 方式获得的也是 char 数组。
...
* 字符串截取问题  
subString 方法是以 char 为准的，而我们已经知道了一个字符可能由1-2个char来存储，如果截取时不考虑这一点，就很容易出问题：
...
```
因为「𫟒」字占两个char，所以使用 subString 截取「𫟒小青童鞋」时，会得到「𫟒小」的结果；
而截取「小青𫟒童鞋」时，则会得到「小青?」的结果，这是因为「𫟒」字被拦腰截断了，而阶段后的第一个字节，Java 无法按照UTF-16 的规则解码，就展示为了问号。

所以想要正确截断字符串，需要利用 offsetByCodePoints(int index, int codePointOffset) 方法，该方法会从执行的 index 开始，往后移动 codePointOffset 个 Unicode 字符，代码如下：
```java
...
特别注意：一旦字符串截取不规范，把2个char的字符从中截断了，然后再用 FastJSON 进行序列化，则会抛出 `com.alibaba.fastjson.JSONException: encodeUTF8 error` 。

这是因为FastJSON 在序列化 Java 字符串时会对合法性做较强的校验，一旦发现不合 UTF-16 规则的，就会抛出异常，详细原因可以参见代码 `com.alibaba.fastjson.util.IOUtils#encodeUTF8` 。


## 3.MySQL 中的 UTF-8
UTF-8 是能支持所有的 Unicode 字符的，因此按理说 emoji 不应该会导致问题，但是在MySQL里实现的 utf8 最长只使用3个字节，如果向一个编码为 utf8 的列中插入一个表情符时，就会报类似`Incorrect string value: '\xF0\xAB\x9F\x92  ...' for column 'name'`
这样的错，可以看出此时想要插入的值已经有四个字节了，三字节已经存不下了。

一种常见误解是以为只有插入emoji时才会报这个错，但其实理解了原因就会知道，只要字符不在BMP平面里都会抛错，比如上例就是尝试向一个 utf8 编码的列中插入「𫟒」时的报的错。

解决方案：
1. 正面解决：修改对应列的编码格式为： utf8mb4
2. 绕过：剔除非BMP平面的字符后再存储。具体实现方案可以利用 Java 提供的 `Character.isSurrogate()` 方法，该方法会识别一个 Java char 是否是 UTF-16 代理，如果是，则说明接下来它存储的是超过BMP平面的字符，需要进行过滤。
代码如下：
```java
